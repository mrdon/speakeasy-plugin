/**
 * XML-RPC client.  See <a href="http://code.google.com/p/json-xml-rpc/wiki/DocumentationForJavaScript">Docs</a> for more info.
 *
 * @public
 */
/*
		...
		if(!useHasOwn || obj.hasOwnProperty(key)){
			...
			if(typeof obj[key] == 'string'){
				...
				if(this.__decodeISO8601 && (matches = obj[key].match(/^(?:(\d\d\d\d)-?(\d\d)(?:-?(\d\d)(?:T(\d\d)(?::(\d\d)(?::(\d\d)(?:\.(\d+))?)?)?)?)?)$/))){
					obj[key] = new Date(0);
                    // debugger;
					if(matches[1]) obj[key].setFullYear(parseInt(matches[1], 10));
					if(matches[2]) obj[key].setMonth(parseInt(matches[2]-1, 10));
					if(matches[3]) obj[key].setDate(parseInt(matches[3], 10));
					if(matches[4]) obj[key].setHours(parseInt(matches[4], 10));
			...
			switch(typeEL.nodeName.toLowerCase()){
				...
				case 'datetime.iso8601':
					...
					if(matches = typeEL.firstChild.nodeValue.match(/^(?:(\d\d\d\d)-?(\d\d)(?:-?(\d\d)(?:T(\d\d)(?::(\d\d)(?::(\d\d)(?:\.(\d+))?)?)?)?)?)$/)){
						if(matches[1]) date.setFullYear(parseInt(matches[1], 10));
						if(matches[2]) date.setMonth(parseInt(matches[2]-1, 10));
						if(matches[3]) date.setDate(parseInt(matches[3], 10));
						if(matches[4]) date.setHours(parseInt(matches[4], 10));
						if(matches[5]) date.setMinutes(parseInt(matches[5], 10));
						if(matches[6]) date.setMilliseconds(parseInt(matches[6], 10));
						return date;
					}
...
rpc.dateToISO8601 = function(date){
	...
	return date.getFullYear()             + '-' +
	       rpc.zeroPad(date.getMonth()+1) + '-' +
		   rpc.zeroPad(date.getDate())    + 'T' +
	       rpc.zeroPad(date.getHours())   + ':' +
		   rpc.zeroPad(date.getMinutes()) + ':' +
		   rpc.zeroPad(date.getSeconds()) + '.' +
		   //Prototype's Date.toJSON() method does not include milliseconds
		   rpc.zeroPad(date.getMilliseconds(), 3);
};
...
/**
 * The version of the underlying XML-RPC library
 */
exports.version = rpc.version;